...
namespace UI_MaBanque
{
    public partial class Add_Opp : Form
    {
        ...
        private void updateSolde(double solde, int id_cpt)
        {
            ...
            SqlConnection c;
            String connString;
            connString = @"Data Source=(localdb)\MSSQLLocalDB;Initial Catalog=BANK;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False";
        ...
        private void ok_Click(object sender, EventArgs e)
        {
            if (mycpt.Text.Length != 0 && hiscpt.Text.Length != 0 && m_op.Text.Length != 0)
            {
                ...
                if (des != null)
                {
                    ...
                    if (v)
                    {
                        this.updateSolde(this.client.lst_compte[ind].solde, this.client.lst_compte[ind].id);
        ...
        private void retrait_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            for (int i = 0; i < this.client.lst_compte.Count; i++)
            {
                lst_cpt.Items.Add(this.client.lst_compte[i].id);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                int ind = this.position_cpt(int.Parse(lst_cpt.Text));
                this.client.lst_compte[ind].debiter(double.Parse(montant.Text));
